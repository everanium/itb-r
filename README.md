...
(`src/libitb3r.c`) driven through R's `.Call` interface. The compiled
shim links `libitb3.so` directly, and every hash-name / MAC-name /
...
`ITB_REBUILD_LIBITB3=1` to force a Go rebuild) and installs the R
...
cd bindings/r && mkdir -p .local && \
    ITB_LIBITB3_DIR="$(pwd)/../../dist/linux-amd64" \
    R CMD INSTALL --no-docs --library=.local .
```

The compiled shim embeds an rpath to the repository's
`dist/linux-amd64` directory, so `libitb3.so` resolves without
`LD_LIBRARY_PATH`; set `ITB_LIBITB3_DIR=<dir>` in the environment
...
```bash
R_LIBS="$PWD/.local" Rscript -e 'library(libitb3r); ...'
```
...
pipeline_save_f(sender, "session.blob")       # written by libitb3, mode 0600
...
`inspect` returns the profile record as the JSON text libitb3 emits;
absent keys are optional fields at their zero value. The package
carries no JSON dependency, so the text is handed over verbatim for
the caller's own decoder (e.g. `jsonlite::fromJSON`).
...
```r
set_memory_limit(4 * 1024 * 1024 * 1024) # 4 GiB soft cap
set_gc_percent(100)                       # balanced GC
```
...
such blobs via `pipeline_load_f`. `itb3` also encrypts / decrypts
