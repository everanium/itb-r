 ...
 * R_registerRoutines (NAMESPACE: useDynLib(libitb3r, .registration = TRUE)).
 *
 ...
 *
 * Handles are R external pointers carrying the libitb3 registry handle
...
/* Wraps a libitb3 handle into a finalized external pointer. `prot` is
...
/* Sorted vector of every registered profile name. libitb3 writes a
...
static int init_thunk(void *ctx, void *out, size_t cap, size_t *n) {
    ...
    /* libitb3 closes an undersized attempt before returning; the retry
...
/* Sets the worker cap; n is clamped by libitb3 (n <= 0 auto, > 256
...
/* Releases the handle (libitb3 closes and zeroes key material first).
...
void R_init_libitb3r(DllInfo *dll);
